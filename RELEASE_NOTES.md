...
`rlp-architect` is a plugin for Devin, Claude Code, Codex, and Cursor (and a set of standalone Agent Skills) for running the **Repository Learning Protocol (RLP)**. RLP turns corrections to AI-agent work into durable repository improvements — checks, tests, scoped context, and skills — so the same mistake is not bought twice.
...
- **Cross-agent install (v0.5).** In the future, the skills may be installable via the open `npx skills` CLI. For v0.4.0, use the native plugin for Devin, Claude Code, Codex, or Cursor, or the manual copy path.
- **MIT license.**
